...
/**@brief Schedule single-param callback execution and unlock
 *        Zigbee task afterwards.
 *
 * Schedule execution of function `func' in the main scheduler loop.
 *
 * @param func    function to execute
 * @param param   callback parameter - usually, but not always
 *                ref to packet buffer
 *
 * @return RET_OK or RET_OVERFLOW.
 */
static inline zb_ret_t zigbee_schedule_callback(zb_callback_t func,
						zb_uint8_t param)
{
...
/**@brief Schedule two-param callback execution and unlock
 *        Zigbee task afterwards.
 *
 * Schedule execution of function `func' in the main scheduler loop.
 *
 * @param func        function to execute
 * @param param       zb_uint8_t callback parameter - usually, but not always
 *                    ref to packet buffer
 * @param user_param  zb_uint16_t user parameter - usually, but not always
 *                    short address
 *
 * @return RET_OK or RET_OVERFLOW.
 */
static inline zb_ret_t zigbee_schedule_callback2(zb_callback_t func,
						 zb_uint8_t param,
 ...
 *        and unlock Zigbee task afterwards.
 *
 * Function will be called via scheduler after timeout
 * expired (maybe, plus some additional time).
 ...
 * @return RET_OK or RET_OVERFLOW
 */
static inline zb_ret_t zigbee_schedule_alarm(zb_callback_t func,
					     zb_uint8_t param,
					     zb_time_t run_after)
{
