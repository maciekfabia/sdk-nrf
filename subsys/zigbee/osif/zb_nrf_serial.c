...
/**
 * Async: Pass partial RX buffer in order to detect and handle RX timeout event.
 *
 ...
 */
static bool handle_rx_timeout(u8_t *buf, size_t len)
{
...
static void uart_evt_handler(struct uart_event *evt, void *user_data)
{
	...
	case UART_RX_RDY:
		...
		/* Async: Since the higher logic expect the RX buffer to be
		 *        fully filled, use UART_RX_BUF_RELEASED to inform about
		 *        RX completion.
		 */
		if (char_handler) {
		...
		/* Async: Detect and handle the RX timeout event. */
		if (uart_rx_buf) {
			restart_rx = handle_rx_timeout(evt->data.rx_buf.buf,
	...
	case UART_RX_BUF_RELEASED:
		if (uart_rx_buf) {
			store_rx_buf(evt->data.rx_buf.buf);
			if (uart_rx_buf_offset == uart_rx_buf_len) {
				uart_rx_buf_len = 0;
