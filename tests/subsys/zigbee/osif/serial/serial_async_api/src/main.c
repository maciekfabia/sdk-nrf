...
K_SEM_DEFINE(rx_lock, 0, 1);

static u8_t uart_rx_test_buf[TEST_UART_BUF_LEN];
static u32_t uart_rx_buf_cnt;
static u8_t uart_rx_mem_test_buf[TEST_UART_BUF_LEN];
...
/**
 * Verify that the driver is found and initialized by the OSIF
 * implementation.
 */
...
/**
 * Verify that the OSIF implementation is ready for transmission and
 * reception.
 */
static void test_tx_rx_setup(void)
{
	...
	zassert_equal_ptr(rx_data_cb, uart_recv_cb,
			  "The UART RX callback in not properly set");
	zassert_equal_ptr(tx_data_cb, uart_send_cb,
			  "The UART TX callback in not properly set");
}
...
static void test_undiv_tx_rx(void)
{
	...
	zassert_equal(0, k_sem_take(&rx_lock, TEST_RX_LOCK_TIMEOUT),
		"Reception was not finished within the RX timeout");
	zassert_equal((bytes_filled / 3) * 3, uart_rx_buf_cnt,
		"The number of transmitted and received bytes is not equal");
	zassert_equal(
		memcmp(test_serial_data, uart_rx_test_buf, uart_rx_buf_cnt),
		0,
...
static void test_rx_timeout_chunk_and_single_byte(void)
{
	...
	for (bytes_filled = 0;
	     bytes_filled < sizeof(test_serial_data);
	     bytes_filled++) {
		test_serial_data[bytes_filled] = (bytes_filled % 0x100);
	}
...
void test_main(void)
{
	...
	ztest_test_suite(nrf_osif_serial_tests,
		...
		ztest_unit_test(test_small_rx_buffer),
		ztest_unit_test(test_single_rx_chunk),
		ztest_unit_test(test_two_rx_chunks),
		ztest_unit_test(test_rx_timeout_single_byte),
		ztest_unit_test(test_rx_timeout_chunk_and_single_byte),
		ztest_unit_test(test_rx_timeout_two_chunks_and_single_byte)
		);
